...
Since that was a dead end, I dropped the accelerometer and tried using a compass magnetometer module to measure heading. Then orientate the robot accordingly if the robot is off course. Long story short, this turned out to be difficult as well. The compass outputs x,y,z axis values. With this I know which direction the robot is facing. However, translating this data into PWM commands to supply to the motor controller to adjust the heading of the robot proved to be challenging. I wanted to use PID control for this but I couldn't establish a link between the IN    PUT (magnetometer's x,y,z values) and the OUTPUT (PWM motor values). Ended up scrapping that idea as well. *Cry cry* Pro tip - Best way to measure distance travelled by a robot is to use motors with encoders! I think I've learnt a very good lesson here :) <br>
